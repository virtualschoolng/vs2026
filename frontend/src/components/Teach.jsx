    ...
    <div className="bg-background font-body">
      <header className="bg-primary text-white">
        <div className="max-w-7xl mx-auto py-20 px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-16 items-center">
          <div className="text-center md:text-left">
            <h1 className="text-5xl font-extrabold text-white font-headline mb-4">Share Your Passion. Shape the Future.</h1>
            <p className="text-xl text-white font-body mb-8">Join our community of expert tutors and empower learners across Nigeria. Earn, inspire, and grow with us.</p>
            <Link to="/register" className="bg-accent text-white font-bold py-4 px-8 rounded-button shadow-lg hover:bg-accent-dark transition-transform transform hover:scale-105 inline-block">
              Become a Tutor Today
            </Link>
          </div>
          <div className="hidden md:flex justify-center">
            <img src={happyTutor} alt="A happy Nigerian tutor teaching online" className="rounded-card shadow-2xl w-full max-w-md object-cover"/>
          </div>
        </div>
      </header>
      ...
      <section className="py-20 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h2 className="text-4xl font-extrabold text-accent font-headline">Why Teach With Us?</h2>
            <p className="mt-4 text-lg text-text-light max-w-3xl mx-auto">We provide the platform, you provide the expertise. Together, we create unparalleled learning experiences.</p>
          </div>
          <div className="mt-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {benefits.map((benefit) => (
              <div key={benefit.title} className="bg-surface p-8 rounded-card shadow-card text-center transform transition-transform hover:-translate-y-2 hover:shadow-lg">
                {benefit.icon}
                <h3 className="text-xl font-extrabold text-accent font-headline mt-6 mb-2">{benefit.title}</h3>
      ...
      <section className="py-20 bg-background">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center">
                <h2 className="text-4xl font-extrabold text-accent font-headline">How to Get Started</h2>
                <p className="mt-4 text-lg text-text-light">Our simple three-step process makes it easy to join our community.</p>
            </div>
            <div className="mt-16 space-y-12">
                {steps.map((step, index) => (
                    <div key={step.title} className="flex flex-col md:flex-row items-center gap-8">
                        <div className="flex-shrink-0 bg-accent-light p-4 rounded-full shadow-md">{step.icon}</div>
                        <div className="text-center md:text-left">
                            <h3 className="text-2xl font-extrabold text-accent font-headline">Step {index + 1}: {step.title}</h3>
      ...
      <section className="py-20 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-extrabold text-accent font-headline">Hear From Our Tutors</h2>
          </div>
          <div className="grid gap-8 md:grid-cols-2">
            {testimonials.map((testimonial) => (
              <div key={testimonial.name} className="bg-surface p-8 rounded-card shadow-card flex flex-col items-center text-center">
                ...
                <p className="text-text-light">{testimonial.subject}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-accent text-white text-center py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-4xl font-extrabold font-headline mb-4">Ready to Inspire?</h2>
            <p className="text-xl text-white mb-8">Join a vibrant community of educators and start making a difference today. Your journey starts here.</p>
            <Link to="/register" className="bg-accent text-white font-bold py-4 px-8 rounded-button shadow-lg hover:bg-accent-dark transition-transform transform hover:scale-105 inline-block">
              Apply to Be a Tutor
            </Link>
        </div>
